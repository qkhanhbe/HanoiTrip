# Chuyển source scan GitLab sang GitHub Actions
...
Thay thế thực thi: [source-scan.yml](../.github/workflows/source-scan.yml), [scan-source.sh](../scripts/ci/scan-source.sh), [security_gate.py](../scripts/ci/security_gate.py). File `gitlab-ci.yml` được giữ làm nguồn đối chiếu, GitHub không thực thi file này.
...
Hiện mới có source-scan. `ci.yml` cho app lint/test/build, Terraform checks/plan, Docker build/image scan sẽ được tạo khi các thành phần đó tồn tại. Terraform plan dùng identity/quyền riêng theo trust boundary của PR.

`cd.yml` sẽ chạy `push` vào `main`: OIDC → build release image SHA → image scan → ACR → staging → smoke → poll production mỗi giây trước swap → swap → xác nhận SHA B/health → rollback và xác nhận SHA A nếu lỗi. CD dùng concurrency riêng, cleanup IP runner tạm bằng `always()` và lưu raw evidence kể cả failure. Source-scan PR-only không có nghĩa tắt CD sau merge.
...
1. Tạo repo bài làm GitHub, push file và mở PR.
...
Chưa có remote GitHub/ruleset/Actions run để đánh dấu các bước trên hoàn tất. Chưa có app, Terraform hoặc triển khai Azure.
